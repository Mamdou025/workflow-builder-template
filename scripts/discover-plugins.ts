...
const requireFromScript = createRequire(
  join(process.cwd(), "scripts", "discover-plugins.ts")
);
...
type RegistryModule = typeof import("../plugins/registry");
type LegacyMappingsModule = typeof import("../plugins/legacy-mappings");

function loadRegistry(): RegistryModule {
  requireFromScript("../plugins/index.ts");
  return requireFromScript("../plugins/registry.ts") as RegistryModule;
}

function loadLegacyMappings(): LegacyMappingsModule {
  return requireFromScript(
    "../plugins/legacy-mappings.ts"
  ) as LegacyMappingsModule;
}
...
function generateIndexFile(plugins: string[]): void {
  const imports = plugins
    .map((plugin) => `import "./${plugin}/index";`)
    .join("\n");
...
function updateReadme(): void {
  const { getAllIntegrations } = loadRegistry();
...
function generateTypesFile(plugins: string[]): void {
  ...
  // Use discovered plugin directories as the source of truth for integration
  // types. This keeps generated types stable even before runtime registration
  // has populated the registry in this process.
...
async function processStepFilesForCodegen(): Promise<void> {
  const { getAllIntegrations, computeActionId } = loadRegistry();
  const integrations = getAllIntegrations();
 ...
 */
function generateStepRegistry(): void {
  const { getAllIntegrations, computeActionId } = loadRegistry();
  const { LEGACY_ACTION_MAPPINGS } = loadLegacyMappings();
  const integrations = getAllIntegrations();
 ...
 */
function generateOutputDisplayConfigs(): void {
  const { getAllIntegrations, computeActionId } = loadRegistry();
  const integrations = getAllIntegrations();
